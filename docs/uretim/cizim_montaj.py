...
def titleblock(fig, title, page, total=4):
    ...
    ax.text(0.02,0.62,"KİNETİK KUM MASASI",fontsize=10,fontweight="bold",transform=ax.transAxes)
    ax.text(0.02,0.22,title,fontsize=8,color="#444",transform=ax.transAxes)
    ax.text(0.64,0.62,"Ölçüler: mm",fontsize=8,transform=ax.transAxes)
    ax.text(0.64,0.22,"Tolerans ±0.5 (aksi belirtilmedikçe)",fontsize=7,color="#444",transform=ax.transAxes)
    ax.text(0.86,0.62,f"Sayfa {page}/{total}",fontsize=8,transform=ax.transAxes)
    ax.text(0.86,0.22,"Rev B",fontsize=8,color="#444",transform=ax.transAxes)
